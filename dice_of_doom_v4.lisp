...
(defun attacking-moves (board cur-player sprae-dice)
  (labels ((player (pos)
                   (car (aref board pos)))
           (dice (pos)
                 (cadr (aref board pos))))
                             ...
                             (lazy-mapcan lambda (dst)
                                          (if (and (not (eq (player dst) cur-player))
                                              ...
                                              (make-lazy (list (list (list src dst)
                                                                     (game-tree (board-attack boar cur-player src dst (dice src))
                                                                                cur-player
                                                                                (+ spare-dice (dice dst))
                                                                                nil)
...
(defun roll-against (src-dice dst-dice)
   (> (roll-dice src-dice) (roll-die ce dst-dice)))
  ...
  (cond ((not pos) (princ "Please choose a hex to move from:"))
        ((eq pos 'pass) (setf *cur-game-tree*
         ...
         (tag a (href (make-game-link nil))
              (princ "continue")))
        ((not *from-tile*) (setf *fromile* pos)
...
(defun add-new-dice (boad player spare-dice)
  (labels ((f (lst n)
              (cond ((zerop n) lst)
                    ((null lst) nil)
